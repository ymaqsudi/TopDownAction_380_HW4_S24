...
export default abstract class NPCAction extends GoapAction {
  ...
  public update(deltaT: number): void {
    ...
    // Check if there is a path and a target
    if (this.path && this.target) {
      // Check if the NPC has reached the target
      if (!this.path.isDone()) {
        // NPC has not reached the target, continue moving on the path
        this.actor.moveOnPath(1, this.path); // Assuming a speed of 1, adjust as necessary
      } else {
        // NPC has reached the target, perform the specific action
        this.performAction(this.target);

        // Optionally, after performing the action, you can clear the target and path
        // to prepare for the next action or to stop the NPC from moving
        this.target = null;
        this.path = null;
      }
    }
